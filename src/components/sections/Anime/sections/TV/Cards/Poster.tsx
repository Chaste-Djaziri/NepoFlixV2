...
import { Card, CardBody, CardFooter, CardHeader, Image, Tooltip } from "@heroui/react";
...
export type AnimeTvPosterCardProps = {
  item: AnimeListItem;         // ← unified
  variant?: "full" | "bordered";
};

const TvShowPosterCard: React.FC<AnimeTvPosterCardProps> = ({ item, variant = "full" }) => {
  ...
  return (
    <>
      ...
      >
        <Link href={`/anime/tv/${item.id}`} ref={ref} {...longPress()}>
          {variant === "full" && (
