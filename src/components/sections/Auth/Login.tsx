    ...
    <div className="flex flex-col gap-5">
      <form className="flex flex-col gap-3" onSubmit={onSubmit}>
        ...
        <div className="flex w-full items-center justify-end px-1 py-2">
          <Link size="sm" className="cursor-pointer text-foreground" onClick={() => setForm("forgot")} isDisabled={isSubmitting}>
            Forgot password?
          </Link>
        </div>
        <Button className="mt-4" color="primary" type="submit" variant="shadow" isLoading={isSubmitting}>
      ...
      <p className="text-center text-small">
        ...
        <Link isBlock size="sm" className="cursor-pointer" onClick={() => setForm("register")} isDisabled={isSubmitting}>
          Sign Up
        </Link>
      </p>
    </div>
